...
// Display CompanyInstance create form on GET.
exports.companyinstance_create_get = function(req, res, next) {
    ...
    .exec(function (err, companys) {
      if (err) { return next(err); }
      // Successful, so render.
      res.render('companyinstance_form', {title: 'Create CompanyInstance', company_list:companys } );
    });

};

// Handle CompanyInstance create on POST.
exports.companyinstance_create_post = [
    ...
    (req, res, next) => {
        ...
        // Create a CompanyInstance object with escaped and trimmed data.
        ...
        if (!errors.isEmpty()) {
            ...
            Company.find({},'title')
                .exec(function (err, companys) {
                    if (err) { return next(err); }
                    // Successful, so render.
                    res.render('companyinstance_form', { title: 'Create CompanyInstance', company_list : companys, selected_company : companyinstance.company._id , errors: errors.array(), companyinstance:companyinstance });
            });
            return;
        }
        else {
...
exports.companyinstance_update_post = [
    ...
    (req, res, next) => {
        ...
        // Create a CompanyInstance object with escaped/trimmed data and current id.
        var companyinstance = new CompanyInstance(
          { company: req.body.company,
            imprint: req.body.imprint,
            status: req.body.status,
